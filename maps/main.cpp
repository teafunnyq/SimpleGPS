...
{
    Navigator ng;
    ng.loadMapData("/Users/tiffanykyu/Desktop/CS32/maps/mapdata.txt");
    vector<NavSegment> directions;
    string s = "1061 Broxton Avenue";
    string s2 = "UCLA Lab School";
    NavResult ok = ng.navigate(s, s2, directions);
    if ( ok == NAV_SUCCESS)
        cout << "Success" << endl;
    else if (ok == NAV_NO_ROUTE)
        cout << "No route"<< endl;
    else if (ok == NAV_BAD_SOURCE)
        cout << "Bad source"<< endl;
    else if (ok == NAV_BAD_DESTINATION)
        cout << "Bad destination"<< endl;
    
    cout << fixed;
    cout << setprecision(3);
    
    double totalM = 0, mForRoad = 0;
    
    if(ok == NAV_SUCCESS)
    {
        cout << "You are starting at: " << s << endl;
        for(int i = 0; i < directions.size(); i++)
        {
            if( directions[i].m_command == NavSegment::PROCEED)
            {
                cout << "Proceed " << directions[i].m_distance << " miles " << directions[i].m_direction << " on " << directions[i].m_streetName << endl;
                mForRoad += directions[i].m_distance;
                totalM += directions[i].m_distance;
            }
            else if (directions[i].m_command == NavSegment::TURN)
            {
                cout << "Turn " << directions[i].m_direction << " onto " << directions[i].m_streetName << endl;
            }

        }
        cout << "You have reached your destination: " << s2 << endl;
        cout << "Total travel distance: " << totalM << " miles" << endl;
    }
}
*/
...
template<typename T>
string directionOfLine(const T& gs)
{
